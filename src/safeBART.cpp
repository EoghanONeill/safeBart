...
//' @export
// [[Rcpp::export]]
NumericMatrix sBART_onefunc_parallel(double lambda,
                                     int num_models,
                                     int num_trees,
                                        int seed,
...
#pragma omp for
  for(int j=0; j<num_models;j++){
    ...
    double templik0=exp(arma::as_scalar((b/2)*log(a)-0.5*log(det(sec_term))-expon*log(nu*lambdaBART - mvm +yty)));
