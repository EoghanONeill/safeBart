...
#' @export
#' @return A vector of variable importances. The variables are ordered in the same order that they occur in columns of the input covariate matrix used to obtain the input BCF-IS object.
#' @examples
#' #set the seed
#' set.seed(100)
#' #simulate some data
...
varImpScores_bcfis<-function(object){
  ...
  class(vIP_mu)<-"varImpScores.bcfIS"
  ...
  class(vIP_tau)<-"varImpScores.bcfIS"
