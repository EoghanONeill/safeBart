...
#' @title Prediction intervals for bart-is output obtained using linear algebra to obtain means and variances, and using bisection to find the quantiles of the mixture of t distributions.
#' 
...
#' @param l_quant Lower quantile of credible intervals
#' @param u_quant Upper quantile of credible intervals
...
predict_bartis <-function(object,#min_possible,max_possible,
  ...
  if(is.null(newdata) ){
    ...
    ret<-pred_ints_exact_outsamp_par(object$sumoftrees,#object$obs_to_termNodesMatrix,
                                     object$ytrain,
                                     object$model_probs,#min_possible, max_possible,
                                     nrow(object$xtrain),
