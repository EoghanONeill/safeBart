...
#' @param object A bartis object obtained using the train_BART_IS or train_BART_IS_no_output function.
#' @export
#' @return A vector of posterior inclusion probabilities. The variables are ordered in the same order that they occur in columns of the input covariate matrix used to obtain the input BART-IS object.
#' @examples
#' #set the seed
#' set.seed(100)
#' #simulate some data
...
varIncProb_bartis<-function(object){
  #object will be bartBMA object.
  imp_vars2=get_weighted_var_imp(num_vars=object$numvars,BIC=object$model_probs,sum_trees=object$sumoftrees)
  #res<-apply((imp_vars2[[3]]>0)*imp_vars2[[1]],2,sum)
  res<- t(imp_vars2[[3]]>0)%*%imp_vars2[[1]]
